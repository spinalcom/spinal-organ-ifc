...
Grammar.prototype.buildSelectList = function(selectName) {
  ...
  if (select_element === null) {
    console.log(" ERROR / buildSelectList : cannot find entity with name ",
      selectName);
  }
...
Grammar.prototype.buildProp = function(entityName, mapTmp) {
  ...
  if (entity === undefined) {
    console.log((" ERROR / buildProp : cannot find entity with name " +
      entityName).red);
  }
  var props = [];
  ...
  function _get_type(composite_type) {
    ...
    assert(false, "cannot find '" + composite_type +
      "' in entities or enumerations");
    return "?";
  }
