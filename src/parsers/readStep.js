...
registerSelect("REPRESENTED_DEFINITION", [
  "PROPERTY_DEFINITION",
...
registerSelect("CHARACTERIZED_DEFINITION", [
  "CHARACTERIZED_PRODUCT_DEFINITION",
  "SHAPE_DEFINITION"
]);
registerSelect("CHARACTERIZED_PRODUCT_DEFINITION", [
  "PRODUCT_DEFINITION",
...
StepIndexer.prototype.parse = function(entity, parser) {
  ...
  if (!parser) {
    if (entity.type) {
      ...
      if (!parser) {
        console.log(
          " Error !!! cannot get parser for ".red,
          entity.type,
          " in ".red,
          type,
          entity
      ...
      entity.types.forEach(function(el) {
        ...
        if (!parser) {
          console.log(
            " Error !!! cannot get parser for ".yellow,
            type,
            " in ".yellow,
            entity
          );
  ...
  stream.on("data", function(data) {
    ...
    if (lines.length >= 2) {
      ...
      if (!pattern_ISO_10303_21.test(lines[0])) {
        my_callback(
          new Error("this file is not a STEP FILE : ISO_10303_21 missing")
        );
      } else {
...
function dumpRecursiveNAUOS(reader, pd, level, node) {
  ...
  sub_parts.forEach(function(rpd) {
    ...
    sub_node.nodes.push({
      label: "" + rpd._id + " " + rpd.formation.of_product.name.yellow.bold
    });
...
StepReader.prototype.dumpAssemblies = function() {
  ...
  ); // util.inspect(sdrs,{depth: 30}));
  sdrs.forEach(function(sdr) {
    var node = {
      label: sdr.definition.definition._id + " SDR:".yellow + sdr.definition
        .name,
      nodes: []
    };
    var ur_node = {
    ...
    if (sdr.definition.definition._class === "PRODUCT_DEFINITION") {
      ...
      var sub_node = {
        label: "PD:".yellow +
          sdr.definition.definition._id +
          " " +
          sdr.definition.definition.formation.of_product.name.cyan,
        nodes: []
      };
      node.nodes.push(sub_node);
      ...
      node.nodes.push({
        label: " relating  = " +
          sdr.definition.definition.relating_product_definition.formation
          .of_product.name
      });
      node.nodes.push({
        label: " related   = " +
          sdr.definition.definition.related_product_definition.formation
          .of_product.name
      });
    ...
    if (sdr.used_representation._class === "SHAPE_REPRESENTATION") {
      ...
      items.forEach(function(item) {
        if (item._class === "MAPPED_ITEM") {
          ...
          var n = {
            label: "MAPPED_ITEM:" + mr._id + " " + mr._class.yellow,
            nodes: []
          };
          ...
          var n = {
            label: item._id + " " + item._class.yellow
          };
          ur_node.nodes.push(n);
      ...
      sdr.used_representation._class ===
      "ADVANCED_BREP_SHAPE_REPRESENTATION"
    ) {
      dumpABSR(sdr.used_representation, ur_node);
    } else {
      ur_node.nodes.push({
        label: "ERROR : ".red + sdr.used_representation
      });
