    ...
    ExpressParser.prototype.parseSimpleStepLine = function(entityName, raw_data,
      obj) {
    ...
    ExpressParser.prototype.parseComplexStepLine = function(entityTypes, obj) {
      ...
      assert(entityTypes instanceof Array,
        " in parseComplexStepLine : entityTypes should be an array");
      assert(entityTypes[0].hasOwnProperty('type'),
        " in parseComplexStepLine : invalid entityTypes");
      assert(entityTypes[0].hasOwnProperty('raw_data'),
        " in parseComplexStepLine : invalid entityTypes");
      ...
      entityTypes.forEach(function(etype) {
        ...
        if (!me.grammar.entities.hasOwnProperty(className)) {
          me.debugLog(" Error !!! cannot get parser for ".yellow,
            className);
        } else {
    ...
    ExpressParser.prototype._parseType = function(obj, raw_data, parser) {
      ...
      "use strict";
      assert(parser !== undefined,
        "expecting valid parser in _parseType with " + raw_data);
      ...
      if (verbose) {
        console.log(entityName, " #", obj._id, "  parsing ".cyan,
          simplePattern, pattern, ' on line ', raw_data);
      }
      var me = this;
      var matches = pattern.exec(raw_data);

      if (!matches) {
        me.debugLog("#", obj._id, entityName, " cannot parse ".red,
          simplePattern, pattern, ' on line ', raw_data);
        return {};
      }
      if (matches.length - 1 !== parser.props.length) {
        console.log(" warning : number element found ".green + matches.length +
          " instead of " + parser.props.length);
